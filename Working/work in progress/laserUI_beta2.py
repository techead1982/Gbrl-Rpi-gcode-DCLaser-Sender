...
class NobleLaser(tk.Tk):  # V0.0.1
    ...
    def Serialset(self):
        self.Sc = SerialClass(write_queue=wq, command_queue=cq, reply_queue=rq)
        self.Sc.start()
        #Sc.start()
...
class GeneralPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.grblleft = tk.Button(self.generalcontrollsframe, text="-X", width=8, height=3, command=lambda : self.jogleft())
        ...
        for port in available_ports:
            ...
            try:
                ...
                while self.ser.inWaiting():
                    ...
                    # print(data)
                    if data.find('Grbl') < 0:
                        # print("No data") rq.put("VER,No Connection")
                        u = 1
                    else:
                        self.grbl_version = data
                        # print(data) rq.put("VER,"+data)
                        i = 1
                        p = port
                        break
                self.ser.close()
